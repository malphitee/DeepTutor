...
export function useCreateRequest(open: () => void) {
  ...
  useEffect(() => {
    if (!query || query.get('create') !== '1') { consumed.current = false; return }
    // A null pathname means the router has not resolved the current page yet.
    // Keep the request pending so the URL is consumed against the real route
    // and workspace scope rather than replacing a placeholder path.
    if (!pathname) return
      ...
      <div className="mt-5 flex justify-end gap-2">
        ...
        <button type="button" className="h-8 rounded-lg bg-primary px-3.5 text-[12px] font-semibold text-primary-foreground hover:opacity-90" onClick={() => {
          ...
          if (destination === activeWorkspaceId()) open()
            ...
            const next = new URLSearchParams((query ?? new URLSearchParams()).toString())
