...
页面和 API 权限以账号的**有效学习策略**为准，不能只检查 `preset=learner`。
Standard 账号也可以受管理员配置的学习策略约束；管理员不受普通学习账号的策略限制。
这类受限账号仍可打开「设置 → 通用 / 外观」，保存语言、主题、界面偏好和语音自动播放
偏好。Learner 预设另保留自己的学习者资料页。

后端按请求方法和完整路径放行个人设置：`GET /api/settings`、
`GET /api/settings/themes`，以及 `PUT /api/settings/ui`、`theme`、`language`、
`voice-autoplay`。`GET/PUT/DELETE /api/settings/draft` 只处理当前账号的草稿；
写入时校验个人界面字段，Learner 预设还可暂存学习者资料。该例外不允许在草稿中夹带
模型目录、工具、工作区等配置，也不会开放完整设置应用接口。

工作区登记、创建、目录迁移和资源分配仍不在有效学习策略的允许范围内。
「学习空间」与策略未开放的学习入口会隐藏；直接访问受限地址会说明该功能由管理员
管理，并提供返回学习或个人设置的入口。已有页面因权限变更收到 403 时，也应显示
权限说明，而不是提示后端未启动。需要工作区管理权限时，请让管理员检查有效学习策略，
不要为修复界面错误而放开整个 `/api/settings/*` 路径。
